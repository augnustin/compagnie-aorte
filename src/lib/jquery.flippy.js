		...
		if(!_Active){
			...
			return this.each(function(){
				...
				switch(_Direction){
					...
					_CenterY = _H/2;
					cv_pattern = '<canvas id="flippy" width="'+(_W+(2*_CenterX))+'" height="'+_H+'"></canvas>';
					...
					_CenterX = _W/2;
					cv_pattern = '<canvas id="flippy" width="'+_W+'" height="'+(_H+(2*_CenterY))+'"></canvas>';
					new_flippy(cv_pattern);
					$this.find("#flippy")
					.css({
						"position":"absolute",
	...
	function toDec(hex){
		...
		for(i=0;i<hexL;i++){
			...
			switch(daHex.toUpperCase()){
				...
				default : dec += daHex*hexPow;break;
			}
		}
		return dec;
	}
