...
var defaults = {
	...
	startToEndCss: function( el, container, endOffset, startOffset ){
		return [
				$.jcoverflip.animationElement( el, { left: ( container.width( )/2 + 160 + 110*endOffset )+'px', bottom: '20px' }, { } ),
	...
	beforeCss: function( el, container, offset ){
		el.removeClass('currentFlip');
		return [
			$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - 160 - 110*offset )+'px', bottom: '20px' }, { } ),
			$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, width: '80px' }, {} )
		];
	},
	...
	currentCss: function( el, container, isFinal ){
		el.addClass('currentFlip');
		if( isFinal ){
			return [
				$.jcoverflip.animationElement( el, { left: ( container.width( )/2 - 50 )+'px', bottom: 0 }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 1, width: '130px' }, { } )
			];
		} else {
			return [
				$.jcoverflip.animationElement( el, { left: ( container.width( )/2 )+'px', bottom: '20px' }, { } ),
				$.jcoverflip.animationElement( el.find( 'img' ), { opacity: 0.5, width: '80px' }, { } )
			];
		}
	},
